...
## Reference data (`public/db/*.json`)

Generated by `tools/build-db.mjs`:

| File | Source | Contents |
|------|--------|----------|
| `skill.json` | `data/luafiles514/lua files/skillinfoz/skillid.lub` + `skillinfolist.lub` (compiled Lua 5.1 bytecode) | Skill ID → display name (pt-BR on the source GRF). 1,354 entries. |
| `mob.json` | rAthena `db/re/mob_db.yml` (canonical) + GRF `npcidentity.lub` (fallback) | Mob ID → `{ name, isBoss, lvl, hp }`. 4,564 entries. |
| `job.json` | `pcjobnamegender.lub` (display) + `admin/pcidentity.lub` (server-side JT → ID) | Job ID → display name. 5,153 entries. The `admin/pcidentity.lub` is critical — Latam server uses non-standard IDs (e.g. `JT_RANGER_H = 4062`, where kRO standard says `JT_MINSTREL = 4062`). |
| `item.json` | `data/idnum2itemdisplaynametable.txt` | Item ID → name. 6,929 entries. Currently unused by the UI. |

Lua bytecode parsing extracts the constant pool walked recursively over nested function prototypes — no Lua VM needed. Server uses a custom-magic GRF (`Event Horizon` instead of `Master of Magic`) with version `0x300` (a 4-byte gap before the file table and 21-byte entry trailers vs standard 0x200's 17 bytes). Mixed-DES-encrypted files (~31k sprites/textures) are skipped — they're irrelevant for stats.

## Caveats

- **Mob spawn names are server-side codes**, not species names. The Latam server sends placeholders like `3I8B` / `2Y8B` for instance mobs. The UI prefers `mob.json`'s species name from `view`/job ID and only falls back to the spawn-packet name. If `mob.json` doesn't have an entry for that view ID (custom server-only mobs), you'll see the raw code.
- **Other players' HP is hidden**: `maxHp = -1` from the server. Mob HP table falls back to rAthena's species-level HP from `mob_db.yml`. Player HP just shows "—".
